...
from .CycleGan import Generator, Generator_Dilated, Generator_Dilated_UNet, Discriminator
...
class Reg_Trainer():#引入多尺度判别器
    def __init__(self, config):
        ...
        # 根据配置选择生成器类型
        generator_type = config.get('generator_type', 'dilated_unet')  # 默认使用带跳跃连接的空洞卷积生成器
        use_dilated_conv = config.get('use_dilated_conv', True)  # 默认使用空洞卷积
        encoder_dilations = config.get('encoder_dilations', [1, 2, 4, 8])
        decoder_dilations = config.get('decoder_dilations', [1, 2, 4])
        
        if generator_type == 'dilated_unet':
            # 🔥🔥🔥 使用带空洞卷积和跳跃连接的 U-Net 生成器（推荐！）
            self.netG_A2B = Generator_Dilated_UNet(
                config['input_nc'], 
                config['output_nc'],
        ...
        elif generator_type == 'dilated':
            # 🔥 使用带空洞卷积的生成器（无跳跃连接）
            self.netG_A2B = Generator_Dilated(
                config['input_nc'], 
                config['output_nc'],
                n_residual_blocks=config.get('n_residual_blocks', 9),
                use_dilated_conv=use_dilated_conv,
    ...
    def train(self):
        ...
        for epoch in range(self.config['epoch']+1, self.config['n_epochs']+1+self.config['decay_epoch']):
            ...
            for i, batch in enumerate(self.dataloader):
                real_A = Variable(self.input_A.copy_(batch['A']))     # CT
                real_B = Variable(self.input_B.copy_(batch['B']))     # CTA
                self.optimizer_R_A.zero_grad()
                self.optimizer_G.zero_grad()
                #### regist sys loss
                fake_B = self.netG_A2B(real_A)                        # 生成出来的CTA
            ...
            #             #fake_B= self.netG_A2B(real_A,real_A).detach().cpu().numpy().squeeze()  原来的代码
            ...
            if epoch % 5 == 0:
                with torch.no_grad():
                    ...
                    for i, batch in enumerate(self.val_data):
                        real_A = Variable(self.input_A.copy_(batch['A']))  ###注意generator输入是[1,3,512,512]
                        real_B = Variable(self.input_B.copy_(batch['B'])).detach().cpu().numpy()
                        fake_B = self.netG_A2B(real_A).detach().cpu().numpy()
                        # fake_B= self.netG_A2B(real_A,real_A).detach().cpu().numpy()  原来的代码
                        # real_B = (real_B * 0.5 + 0.5) * 255
                        # fake_B = (fake_B * 0.5 + 0.5) * 255
                        # 遍历 batch 中的每个样本，逐个计算 PSNR 和 SSIM
                        for j in range(real_B.shape[0]):
                            real_B_j = real_B[j].squeeze()  # (512, 512)
                            fake_B_j = fake_B[j].squeeze()  # (512, 512)
                            psnr = self.PSNR(fake_B_j, real_B_j)  # fake_B
                            PSNR += psnr
                            ssim = measure.compare_ssim(fake_B_j, real_B_j)
                            SSIM += ssim
                            num += 1
                    print('PSNR:', PSNR / num)
                    print('SSIM:', SSIM / num)
    ...
    def test(self,):
        self.netG_A2B.load_state_dict(torch.load(self.config['save_root'] + 'netG_A2B_60_29.5803_0.885.pth'))#43netG_A2Ba512.pth netG_A2Bc512.pth
